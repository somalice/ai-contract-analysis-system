<template>
  <!-- 生成记录详情页:生成信息 / AI 补充条款 / RAG 引用 / 校验结果 / Agent Trace Timeline / 下载 -->
  <div class="page-container" v-loading="loading">
    ...
    <el-card class="action-card mb-16" shadow="never">
      <div class="action-bar">
        ...
        <div class="action-right">
          <el-button
            v-if="generation?.status === 'success' && generation?.contract_id"
            :icon="Document"
            @click="goContract"
          >
            查看合同
          ...
          <el-button
            v-if="generation?.status === 'success'"
            type="primary"
            :icon="Download"
    ...
    <template v-if="generation">
      ...
      <el-card class="mb-16 status-overview" shadow="never" :class="overviewCardClass">
        <div class="overview-content">
          <div class="overview-left">
            ...
            <div class="overview-level">
              <el-tag
                :type="GENERATION_STATUS_TAG_TYPES[generation.status] || 'info'"
                effect="dark"
                size="large"
              >
          ...
          <div class="overview-right">
            <div class="overview-stat">
              <span class="stat-label">补充条款</span>
              <span class="stat-value">{{ generation.generated_clauses?.length || 0 }}</span>
            </div>
            <div class="overview-stat">
              <span class="stat-label">RAG 命中</span>
              <span class="stat-value info">{{ generation.rag_references?.length || 0 }}</span>
            </div>
            <div class="overview-stat">
              <span class="stat-label">迭代次数</span>
              <span class="stat-value">{{ generation.iterations ?? 0 }}</span>
            </div>
            <div class="overview-stat">
              <span class="stat-label">校验</span>
              <el-tag
                v-if="generation.validation_results"
                :type="generation.validation_results.passed ? 'success' : 'warning'"
                size="small"
              >
                {{ generation.validation_results.passed ? '通过' : '未通过' }}
              </el-tag>
              <span v-else>-</span>
            </div>
          </div>
        ...
        <!-- 失败原因 -->
        <el-alert
          v-if="generation.status === 'failed' && generation.error_message"
          type="error"
          :closable="false"
          show-icon
          class="mt-12"
        >
          <template #title>生成失败:{{ generation.error_message }}</template>
        </el-alert>
        <!-- LLM 降级提示 -->
        <el-alert
          v-if="generation.llm_error"
          type="warning"
          :closable="false"
          show-icon
          class="mt-12"
        >
          <template #title>
            LLM 不可用({{ generation.llm_error_type || 'unknown' }}),Agent 已降级为规则校验模式
          </template>
          <div>{{ generation.llm_error }}</div>
        </el-alert>
      </el-card>
      ...
      <el-card class="mb-16" shadow="never">
        ...
        <el-descriptions :column="3" border>
          <el-descriptions-item label="生成编号">{{ generation.generation_no }}</el-descriptions-item>
          <el-descriptions-item label="使用模板">
            <span v-if="generation.template">{{ generation.template.name }}</span>
            <span v-else>-</span>
          </el-descriptions-item>
          <el-descriptions-item label="创建的合同">
            <el-link v-if="generation.contract" type="primary" :underline="false" @click="goContract">
              {{ generation.contract.title }} ({{ generation.contract.contract_no }})
            </el-link>
            <span v-else class="text-muted">(预览,未建合同)</span>
          </el-descriptions-item>
          <el-descriptions-item label="开始时间">{{ formatTime(generation.started_time) }}</el-descriptions-item>
          <el-descriptions-item label="完成时间">{{ formatTime(generation.finished_time) }}</el-descriptions-item>
          <el-descriptions-item label="创建时间">{{ formatTime(generation.created_time) }}</el-descriptions-item>
          <el-descriptions-item label="文件大小" :span="3">
            {{ generation.file_info ? formatFileSize(generation.file_info.size) : '-' }}
          </el-descriptions-item>
        </el-descriptions>

        <!-- 用户填写的变量 -->
        <el-card v-if="generation.input_variables && Object.keys(generation.input_variables).length" shadow="never" class="section-card">
          <template #header><span>用户填写的变量</span></template>
          <el-descriptions :column="2" border>
            <el-descriptions-item
              v-for="(val, key) in generation.input_variables"
              :key="key"
              :label="String(key)"
            >
              {{ val || '(空)' }}
      ...
      <!-- AI 补充条款 -->
      <el-card v-if="generation.generated_clauses?.length" class="mb-16" shadow="never">
        <template #header>
          <div class="card-header">
            <el-icon><MagicStick /></el-icon>
            <span>AI 补充条款</span>
            <el-tag size="small" type="info" class="header-tag">
              {{ generation.generated_clauses.length }} 条
            </el-tag>
          </div>
        </template>
        <el-collapse>
          <el-collapse-item
            v-for="(c, idx) in generation.generated_clauses"
            :key="idx"
            :name="idx"
          >
            <template #title>
              <span class="clause-title">
                第{{ idx + 1 }}条 {{ c.name }}
                <el-tag size="small" :type="c.source === 'ai' ? 'primary' : 'success'" class="ml-8">
                  {{ c.source === 'ai' ? 'AI 生成' : '知识库' }}
                </el-tag>
              </span>
            </template>
      ...
      <!-- RAG 命中规范 -->
      <el-card v-if="generation.rag_references?.length" class="mb-16" shadow="never">
        <template #header>
          <div class="card-header">
            <el-icon><Collection /></el-icon>
            <span>RAG 命中规范</span>
            <el-tag size="small" type="info" class="header-tag">
              {{ generation.rag_references.length }} 条
            </el-tag>
          </div>
        </template>
        <el-table :data="generation.rag_references" stripe border>
      ...
      <!-- 校验结果 -->
      <el-card
        v-if="generation.validation_results && !generation.validation_results.passed"
        class="mb-16"
        shadow="never"
      >
        <template #header>
          <div class="card-header">
            ...
            <el-tag size="small" type="warning" class="header-tag">
              {{ generation.validation_results.issues?.length || 0 }} 项
            </el-tag>
          </div>
        </template>
        <el-table :data="generation.validation_results.issues" stripe border>
      ...
      <!-- Agent 执行过程 Trace -->
      <el-card
        v-if="generation.agent_trace && generation.agent_trace.length > 0"
      ...
      >
        <template #header>
          <div class="card-header">
            ...
            <el-tag size="small" type="info" class="header-tag">
              {{ generation.agent_trace.length }} 步 · {{ generation.iterations ?? 0 }} 次迭代
            </el-tag>
          </div>
        </template>

        <!-- Trace 汇总统计 -->
        <div v-if="generation.trace_summary" class="trace-summary-bar">
          <div class="trace-stat-item">
            <span class="trace-stat-label">总步数</span>
            <span class="trace-stat-value">{{ generation.trace_summary.steps ?? generation.agent_trace.length }}</span>
          </div>
          <div class="trace-stat-item">
            <span class="trace-stat-label">总耗时</span>
            <span class="trace-stat-value">{{ generation.trace_summary.total_duration_ms ?? 0 }} ms</span>
          </div>
          <div class="trace-stat-item">
            <span class="trace-stat-label">LLM 耗时</span>
            <span class="trace-stat-value primary">{{ generation.trace_summary.llm_duration_ms ?? 0 }} ms</span>
          </div>
          <div class="trace-stat-item">
            <span class="trace-stat-label">Tool 耗时</span>
            <span class="trace-stat-value success">{{ generation.trace_summary.tool_duration_ms ?? 0 }} ms</span>
          </div>
        </div>
        ...
        <el-timeline class="agent-timeline">
          <el-timeline-item
            v-for="step in generation.agent_trace"
            :key="step.step"
      ...
      <!-- Agent 工具调用轨迹 -->
      <el-card v-if="generation.trace_summary?.tool_stats" shadow="never">
        <template #header>
          <div class="card-header">
...
<script setup>
/**
 * 生成记录详情页(Sprint 6 - v0.8.0)
 *
 * 展示内容:
 * 1. 生成状态总览(status / 补充条款数 / RAG 命中数 / 迭代次数 / 校验结果)
 * 2. 生成元信息(生成编号 / 模板 / 合同 / 时间 / 文件大小)
 * 3. 用户填写的变量
 * 4. AI 补充条款(可折叠,含 RAG 引用来源)
 * 5. RAG 命中规范(文档 / 页码 / 相似度)
 * 6. 校验问题(若校验未通过)
 * 7. Agent 执行过程 Trace(Timeline:Thought → Decision → Action → Observation)
 * 8. 工具调用统计
 * 9. 下载 Word 文档
 */
...
import { getGenerationDetail, downloadGeneratedContract } from '@/api/generation'
import {
...
const toolStatsList = computed(() => {
  const stats = generation.value?.trace_summary?.tool_stats
...
async function fetchDetail() {
  loading.value = true
  try {
    const res = await getGenerationDetail(route.params.id)
    generation.value = res.data.generation
  } catch (e) {
    // 错误提示由拦截器统一处理
  } finally {
    loading.value = false
...
async function handleDownload() {
  ...
  try {
    const res = await downloadGeneratedContract(generation.value.id)
    const url = window.URL.createObjectURL(res)
    const a = document.createElement('a')
    a.href = url
    a.download = generation.value.contract?.title
      ? `${generation.value.contract.title}.docx`
      : `${generation.value.generation_no}.docx`
...
function goContract() {
  if (generation.value?.contract_id) {
    router.push(`/contracts/${generation.value.contract_id}`)
  }
}

// ---------- Agent Trace 辅助函数(复用 ReviewDetail 模式) ----------
