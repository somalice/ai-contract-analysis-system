"""
Proposal Agent 执行结果(Sprint 7 - v0.9.0)

职责:
- 承载 ProposalAgent.run() 的产物:生成章节 / RAG 引用 / 校验结果 / Trace

设计:
- 与 Sprint 5 AgentResult / Sprint 6 GenerationResult 同形(status / iterations /
  llm_error / llm_error_type / tool_calls_log / agent_trace / trace_summary),
  便于前端复用 Timeline 组件
- 投标专属字段:generated_sections / rag_references / validation_results
- 不修改 Sprint 5/6 Result 类(领域不同,独立类)
"""
...
class ProposalResult:
    ...
    # 状态枚举(与 GeneratedProposal 状态机一致)
    SUCCESS = 'success'
    FAILED = 'failed'

    def __init__(self, status: str,
                 generated_sections: Optional[list] = None,
        ...
        """
        self.status = status
        self.generated_sections = generated_sections or []
    ...
    def __repr__(self) -> str:
        return (
            f'<ProposalResult {self.status} sections={len(self.generated_sections)} '
